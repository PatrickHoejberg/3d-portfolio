...
export function CursorDrivenParticleTypography({
    ...
    useEffect(() => {
        ...
        const init = () => {
            ...
            // Measure the text at the requested fontSize, then scale it down
            // so it actually fits inside the container — width AND height.
            const maxTextWidth = containerWidth * 0.92;   // 4% side padding
            const maxTextHeight = containerHeight * 0.6;  // keep clear of top/bottom
            ...
            if (measuredWidth > maxTextWidth) {
                effectiveFontSize = fontSize * (maxTextWidth / measuredWidth);
            }
            effectiveFontSize = Math.min(effectiveFontSize, maxTextHeight);

            // How much the font had to shrink relative to the requested size.
            // 1 = no shrink, closer to 0 = heavily shrunk (e.g. small phones).
            const fontScaleRatio = Math.min(1, effectiveFontSize / fontSize);

            ctx.font = `bold ${effectiveFontSize}px ${fontFamily}`;

            // Draw with the fitted font size
            ...
            // Sample more finely as the font shrinks, so small text
            // (e.g. on narrow phones) still has enough dots to read clearly.
            const step = Math.max(1, Math.floor(particleDensity * fontScaleRatio * dpr));

            // Keep dots from getting too tiny/sparse-looking when the font
            // has shrunk a lot, without letting them get oversized at full scale.
            const effectiveParticleSize = Math.max(0.8, particleSize * Math.min(1, fontScaleRatio + 0.3));
            ...
            for (let y = 0; y < textCoordinates.height; y += step) {
                for (let x = 0; x < textCoordinates.width; x += step) {
                    ...
                    if (alpha > 128) {
                        particles.push(
                            new Particle(x / dpr, y / dpr, effectiveParticleSize, textColor, dispersionStrength, returnSpeed)
                        );
                    }
                }
            }
